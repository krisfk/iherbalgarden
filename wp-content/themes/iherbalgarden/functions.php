...
add_action( 'rest_api_init', function () {
	register_rest_route( 'api', '/school-login', array(
	  'methods' => 'POST',
	  'callback' => 'school_login',
	) );

	register_rest_route( 'api', '/school-logout', array(
		'methods' => 'POST',
		'callback' => 'school_logout',
	  ) );


	register_rest_route( 'api', '/like-school', array(
		'methods' => 'POST',
		'callback' => 'like_school',
	  ) );

	  register_rest_route( 'api', '/watch-post', array(
		'methods' => 'POST',
		'callback' => 'watch_post',
	  ) );
...
function watch_post($request)
{
	$post_id= $request->get_param( 'post_id' );
	
	$watch_posts_str=get_field('watched_posts',$_SESSION['school_id']);

	$pieces = explode(",", $watch_posts_str);


	if (in_array($post_id, $pieces)) {
		echo json_encode(array("status"=>"2", "msg"=>"watched this post"));
	}
	else
	{
		...
		if(!$watch_posts_str)
		{
			update_field('watched_posts', $post_id,$_SESSION['school_id'] );
		}
		else
		{
			update_field('watched_posts', $watch_posts_str.','.$post_id,$_SESSION['school_id'] );
		}
			echo json_encode(array("status"=>"3", "msg"=>"added this post to be watched"));

	}

	// update_field('watched_posts', $value, [$post_id]);

	

}
 
function like_school($request)
{
...
function school_logout($request)
{
	$_SESSION['school_id']='';
	$_SESSION['school_account_id']='';
	wp_logout();
	echo json_encode(array("status"=>"1", "msg"=>"已登出！"));
}
  
  function school_login($request)
{
		$login_name= $request->get_param( 'login_name' );
		$login_pw= $request->get_param( 'login_pw' );
		...
		// echo $user->data->ID;
		if ( is_wp_error( $user ) ) {
			 echo json_encode(array("status"=>"-1", "msg"=>"登入資料不正確。"));
		}
		else
		{		
			...
			// echo 999;
			   ...
			   if ( $query->have_posts() ) {   			
				while ( $query->have_posts() ) {
				   ...
				   $query->the_post();
				//    echo 1111;
				   if(get_field('school_account')['user_nicename']==$login_name)
				   {
					$_SESSION['school_account_id']=$user->data->ID;

					   $_SESSION['school_id']=get_the_ID();
					//    echo get_the_ID();
				   }
